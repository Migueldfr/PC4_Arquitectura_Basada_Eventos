...
TABLE_NAME = os.environ.get('TABLE_NAME', 'alpinista_events')
ALARMAS_TABLE = os.environ.get('ALARMAS_TABLE', 'weather_alarmas')
ALLOWED_EVENTS = os.environ.get('ALLOWED_EVENTS', 'temperature-sensor,wind-sensor').split(',')
CONSUMIDOR = os.environ.get('CONSUMIDOR', 'Alpinista')
...
def simular_fallo_aleatorio():
    ...
    if numero % 2 == 0 and numero > 80:  # 82, 84, 86, 88, 90, 92, 94, 96, 98, 100
...
def procesar_alarma(item):
    ...
    try:
        ...
        print(f"⚠️  Error guardando alarma en DynamoDB: {e}")
...
def lambda_handler(event, context):
    ...
    print(f"🏔️ [{CONSUMIDOR}] Procesando batch de {len(event['Records'])} mensajes")
    
    for record in event['Records']:
        try:
            # 1. Parsear SQS → SNS → Mensaje
            body = json.loads(record['body'])
            
            # SNS envuelve el mensaje en 'Message'
            if 'Message' in body:
            ...
            # 2. Filtrar eventos no permitidos
            if event_type not in ALLOWED_EVENTS:
                print(f"⚠️  Ignorando evento no permitido: {event_type}")
                continue
            
            # 3. Chaos Monkey (solo para testing, comentar en producción)
            simular_fallo_aleatorio()
            
            # 4. Convertir floats a Decimal (requerido por DynamoDB)
            item = json.loads(json.dumps(message), parse_float=Decimal)
            
            # 5. Mapear campos (Producer usa camelCase, DynamoDB usa snake_case)
            item['event_id'] = item.get('eventId')
            item['event_type'] = item.get('eventType')
            
            # 6. Procesar alarma si es extremo
            procesar_alarma(item)
            
            # 7. Guardar en tabla principal
            table.put_item(Item=item)
            print(f"✅ [{CONSUMIDOR}] Guardado: {event_type} - {event_id}")
            
        except Exception as e:
            print(f"❌ Error procesando mensaje: {e}")
            # Lanzar excepción para que SQS reintente
            raise
