...
def procesar_alarma(event_id, event_type, valor, unidad, status, timestamp_evento):
    ...
    condiciones_extremas = ["EXTREME_COLD", "EXTREME_HEAT", "EXTREME", "STORM","CRITICAL","PELIGROSA","MUY_INSALUBRE","INSALUBRE"]
    ...
    try:
        cloudwatch.put_metric_data(
            Namespace='WeatherEvents/Ciclista',
            MetricData=[{
    ...
    try:
        ...
        ''', (event_id, event_type, timestamp_evento, valor, unidad, status, 'Ciclista'))
        conn.commit()
        conn.close()
...
# ============ CONFIGURACIÓN ============
QUEUE_URL = os.getenv("SQS_CICLISTA_URL", f"https://sqs.eu-central-1.amazonaws.com/{os.getenv('ACCOUNT_ID')}/weather_ciclista")
...
# ============ LÓGICA ESPECÍFICA ============
# El Ciclista procesa Calidad del Aire y Visibilidad
TARGET_TABLE = "ciclista_data"
...
def extract_data(clima_event):
    ...
    if event_type == "AirQuality-sensor":
        valor = data.get('aqi')
        status = data.get('category') # Para aire usa category como status
...
def guardar_en_mysql(clima_event):
    ...
    if event_type not in ALLOWED_EVENTS:
        print(f"⚠️  Ignorando evento no relevante para Ciclista: {event_type}")
        return
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"✅ [Ciclista] Guardado: {event_type} - {valor} {unidad}")
