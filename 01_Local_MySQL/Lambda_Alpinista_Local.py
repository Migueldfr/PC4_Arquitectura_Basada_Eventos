...
def simular_fallo_aleatorio():
    ...
    numero = random.randint(30, 40)  # Genera un número entre 1 y 100
    if numero % 2 == 0 and numero > 30:  # Si es par Y > 80 (82, 84, 86, 88, 90, 92, 94, 96, 98, 100)
...
def procesar_alarma(event_id, event_type, valor, unidad, status, timestamp_evento):
    ...
    # Lista de condiciones extremas
    condiciones_extremas = ["EXTREME_COLD", "EXTREME_HEAT", "EXTREME", "STORM","CRITICAL","PELIGROSA"]
    
    if status not in condiciones_extremas:
        return  # No es extremo, no hacer nada
    
    # 1. Enviar a CloudWatch
    try:
        cloudwatch.put_metric_data(
            Namespace='WeatherEvents/Alpinista',
            MetricData=[
                {
    ...
    # 2. Guardar en tabla alarmas de MySQL
    try:
        ...
        ''', (event_id, event_type, timestamp_evento, valor, unidad, status, 'Alpinista'))
...
# ============ CONFIGURACIÓN ============
QUEUE_URL = os.getenv("SQS_ALPINISTA_URL", f"https://sqs.eu-central-1.amazonaws.com/{os.getenv('ACCOUNT_ID')}/weather_alpinista")
...
# ============ LÓGICA ESPECÍFICA ============
# El Alpinista solo procesa Temperatura y Viento
TARGET_TABLE = "alpinista_data"
...
def extract_data(clima_event):
    ...
    if event_type == "temperature-sensor":
        valor = data.get('value')
        unidad = data.get('unit')
    elif event_type == "wind-sensor":
...
def guardar_en_mysql(clima_event):
    ...
    if event_type not in ALLOWED_EVENTS:
        print(f"⚠️  Ignorando evento no relevante para Alpinista: {event_type}")
        return
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"✅ [Alpinista] Guardado: {event_type} - {valor} {unidad}")
...
def main():
    print("=" * 50)
    print("🏔️  LAMBDA LOCAL: ALPINISTA")
    print("=" * 50)
